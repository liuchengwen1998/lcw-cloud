package com.lcw.cloud.redis.util;
...
public class RedisUtils {
    ...
    public static void setRedisTemplate(RedisTemplate<String, Object> redisTemplate0) {
    ...
    /**
     * 批量缓存放入Redis String类型
     * 默认不过期
     *
     * @param beans 批量保存的对象 map key 就是redis key
     * @return true成功 false失败
     */
    public static boolean pipelinedSet(Map<String, Object> beans) {
        return pipelinedSet(beans, -1L);
    }
     ...
     * @return true成功 false失败
     */
    public static boolean pipelinedSet(Map<String, Object> beans, long time) {
        try {
            RedisSerializer keyS = redisTemplate.getKeySerializer();
            RedisSerializer valueS = redisTemplate.getValueSerializer();
            redisTemplate.executePipelined((RedisCallback<String>) connection -> {
                beans.forEach((k, v) -> {
                    connection.set(keyS.serialize(k), valueS.serialize(v)
