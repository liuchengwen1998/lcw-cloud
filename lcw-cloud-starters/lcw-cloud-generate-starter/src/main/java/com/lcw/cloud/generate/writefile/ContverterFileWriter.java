 ...
 * @since 1.0
 */
public class ContverterFileWriter extends BaseFileWriter{
  ...
  protected Set<String> getImports(ModuleInfo moduleInfo, String className, Map<String, String> packages) {
    ...
    moduleInfo.getApis().get(className).forEach(apiInfo -> {
      ...
      imports.add(packages.get("PO")+"."+apiInfo.getTableName()+"PO");
    });
    return imports;
  }
  ...
  protected void write(ModuleInfo moduleInfo, VelocityEngine ve, VelocityContext ctx, Map<String, String> packages) {
    moduleInfo.getApis().forEach((k, v) -> {
      ...
      write(moduleInfo,k,ve, ctx,packages,getFileName(packages, StringUtils.capitalize(moduleInfo.getSubSystemCode()) + StringUtils.capitalize(k)+"POConverter"));
    });
  }

  private String getFileName(Map<String, String> packages,String fileName ) {
    return packages.get("Converter").replaceAll("\\.", Matcher.quoteReplacement(File.separator)) + File.separator + fileName +".java";
  }

  @Override
  public String getTemplate(String subSystemCode) {
    if (StringUtils.isNotBlank(subSystemCode)) {
      return "api/converter.java.vm";
    } else {
      return "api2/converter.java.vm";
    }
  }
