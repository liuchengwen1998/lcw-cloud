 ...
 * @since 1.0
 */
public class FeignFileWriter extends BaseFileWriter {
    ...
    protected void write(ModuleInfo moduleInfo, VelocityEngine ve, VelocityContext ctx, Map<String, String> packages) {
        moduleInfo.getApis().forEach((k, v) -> {
            ...
            write(moduleInfo, k, ve, ctx, packages,
                    getFileName(packages, StringUtils.capitalize(moduleInfo.getSubSystemCode()) + StringUtils.capitalize(k) + "Client"));
        });
    }

    private String getFileName(Map<String, String> packages, String fileName) {
        return packages.get("Feign").replaceAll("\\.", Matcher.quoteReplacement(File.separator)) + File.separator + fileName + ".java";
    }

    @Override
    public String getTemplate(String subSystemCode) {
        if (StringUtils.isNotBlank(subSystemCode)) {
            return "api/feign.java.vm";
        } else {
            return "api2/feign.java.vm";
        }
    }
