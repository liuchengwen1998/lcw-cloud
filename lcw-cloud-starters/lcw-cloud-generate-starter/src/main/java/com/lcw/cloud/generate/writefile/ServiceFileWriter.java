 ...
 * @since 1.0
 */
public class ServiceFileWriter extends BaseFileWriter{
  ...
  protected void write(ModuleInfo moduleInfo, VelocityEngine ve, VelocityContext ctx, Map<String, String> packages) {
    moduleInfo.getApis().forEach((k, v) -> {
      ...
      write(moduleInfo,k,ve, ctx,packages,getFileName("Service",packages,StringUtils.capitalize(moduleInfo.getSubSystemCode()) + StringUtils.capitalize(k) +"Service"));
    });
  }
  ...
  public String getTemplate(String subSystemCode) {
    if (StringUtils.isNotBlank(subSystemCode)) {
      return "api/service.java.vm";
    } else {
      return "api2/service.java.vm";
    }
